function(stm32_extract_info identifier)
    ...
    string(REGEX MATCH "^(STM32)?([CFGHLNU][01234567]|MP[12]|N6|WL3?|WB[0A]?)([0-9A-Z][0-9M]?)?([A-Z135])?([03468ABCDEFGHIJYZ])?(-[AX])?_?(M0PLUS|M33|M4|M7)?.*$" ID ${identifier})
    if(NOT ID)
        return()
    endif()
    ...
    set(EXTENSION ${CMAKE_MATCH_6})
    set(CORE ${CMAKE_MATCH_7})
    ...
    if (INFO_DEVICE AND FAMILY AND SUB_FAMILY AND PIN_COUNT AND NOT "${FLASH_SIZE}" STREQUAL "")
        set(${INFO_DEVICE} ${FAMILY}${SUB_FAMILY}${PIN_COUNT}${FLASH_SIZE}${EXTENSION} PARENT_SCOPE)
    endif()

    if (INFO_FLASH_CODE AND NOT "${FLASH_SIZE}" STREQUAL "")
...
function(stm32_get_memory_info)
    ...
    string(TOLOWER ${INFO_FAMILY} FAMILY_L)
    # Prototype is:
    #   - single core: stm32<family>_get_memory_info(DEVICE TYPE FLASH RAM)
    #   - multi core: stm32<family>_get_memory_info(DEVICE TYPE CORE FLASH FLASH_ORIGIN RAM RAM_ORIGIN TWO_FLASH_BANKS)
    if(COMMAND "stm32${FAMILY_L}_get_memory_info")
        if(INFO_CORE)
            cmake_language(CALL stm32${FAMILY_L}_get_memory_info ${INFO_DEVICE} ${INFO_TYPE} "${INFO_CORE}" FLASH FLASH_ORIGIN RAM RAM_ORIGIN TWO_FLASH_BANKS)
        else()
            cmake_language(CALL stm32${FAMILY_L}_get_memory_info ${INFO_DEVICE} ${INFO_TYPE} FLASH RAM)
        endif()
    endif()
